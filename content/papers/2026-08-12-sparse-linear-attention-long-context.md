...
authors: "Haoyi Zhu（两篇博客主线）；相关论文作者见文末"
venue: "主题阅读笔记（Blog + Papers）"
published: "2026"
...
one_liner: "面对超长上下文，Sparse Attention 精确保留少数 top-K 尖峰，Linear Attention 低成本覆盖大面积长尾；更好的答案是让二者分工。"
...
1. 随着 token 上下文变得非常长，Full Attention 肯定不能一直这样算下去。为了解决这个问题，目前工业界有两条重要路线：**Sparse Attention 系列**与 **Linear Attention 系列**。
2. Sparse Attention 和 Linear Attention 各有明显缺点，但两边的缺点恰好互补。如何把它们组合起来，可能是实现 long-context attention、同时保持效率和精度的关键。
...
> [!IMPORTANT]
> 本文的核心结论是：**Sparse Attention 用精确计算保住少数尖峰，Linear Attention 用低成本近似覆盖大面积长尾。更自然的组合是“top-K 精算，其余位置近似”，而不是在二者之间二选一。**

![Full Attention、Sparse Attention、Linear Attention 与二者结合的信息流](media/sparse-linear-attention-long-context/01-sparse-linear-overview.png "图 1｜从 Full Attention 到 Sparse–Linear Attention：Sparse 选择少数 top-K 精确计算，Linear 把全部历史压入 recurrent state，二者结合后分别处理尖峰与长尾。来源：OpenAI 内置图像生成能力按 littlewei 的笔记生成；版权：littlewei，All Rights Reserved")
...
- **[DSA](https://arxiv.org/abs/2512.02556)：token 级。** 对每个 query 选择 top-$K$ KV token。
- **[MoBA](https://arxiv.org/abs/2502.13189)：block 级。** 先计算 query 与 block 表示的相关性，再选择 top-$k$ blocks。
...
DSA、MoBA、VSA 的本质并没有变化：只是分别在 **token、block 和 cube** 三个粒度上做 top-K。

**Sparse Attention 的问题**

可以看到，稀疏注意力的核心就是：**低成本估算 top-K，然后只在 top-K 位置算精确 softmax attention，其余位置全部丢弃。**

这样计算复杂度固然降下来了。LLM 中 top-$K$ 可以取 2048 这个量级；视频生成工作里也经常只保留约 $10\%$ 左右的精确计算，不过具体密度因模型和实验设置而异，例如 VSA 的核心设置常用 $12.5\%$。

问题是：绝大部分位置的信息被直接丢弃，未免有些浪费，也不够精确。那些单个权重不高、但数量庞大的“长尾”位置，加起来的贡献可能并不可忽略。

> [!NOTE]
> 这里所说的“精确 attention”，准确含义是**在选中的集合内部精确计算 softmax**，并不等于恢复了 Full Attention。未选位置不仅从分子中消失，也从 softmax 分母中消失；此外，路由器还可能漏掉真正关键的 token。
...
括号中的 key–value 统计量可以提前累积，不再需要为每个 query 与所有 key 显式计算 pairwise attention。若把 head dimension 视为常数，复杂度随序列长度为 $O(L)$；更严格地写，常见实现约为 $O(Ld^2)$，recurrent state 的大小约为 $O(d^2)$。
...
因此，Linear Attention 虽然在序列长度上非常便宜，但它本质上用固定统计量或状态替代了完整 softmax interaction，一定会损失信息。它最擅长的是大面积、相对平坦的 attention 区域，最不擅长的恰好是少数 sharp peaks。

**KDA 系列：Vanilla → Delta → GDN → KDA**
...
其中第一项会沿当前 key $k_t$ 的方向擦除旧内容，第二项再写入新的 value。也就是说，Delta 不再只是累加，而是会根据当前输入修改已有记忆。
...
![Vanilla Linear、Delta Rule、GDN 与 KDA 的状态更新区别](media/sparse-linear-attention-long-context/02-kda-evolution.png "图 2｜Vanilla Linear 只累加；Delta Rule 定向擦除并写入；GDN 先全局遗忘再修改；KDA 把遗忘细化到通道。来源：OpenAI 内置图像生成能力按 littlewei 的笔记生成；版权：littlewei，All Rights Reserved")
...
- TTT 使用 dual form、mini-batch 等方式降低开销，所以“每个 token 都做一次普通大模型反向传播”也不准确。

我的简化理解仍然可以保留：**KDA 是被高度结构化、工程效率很高的在线更新；TTT 是更一般、更精细，但通常也更贵的在线更新。**
...
### 2.1 成功案例 1：LLM 中使用 KDA + MLA

在 Kimi Linear 的架构中，KDA 与 Full MLA 以 **3:1** 的比例交替工作：
...
KDA 本身已经通过 recurrent state、decay、causal recurrence 和 ShortConv 引入了顺序与局部性。Kimi Linear 进一步在 Full MLA layers 中使用 NoPE，因此不存在“RoPE 超过训练长度后，频率本身如何外推”的直接问题。

不过这里需要保留一个限定：**NoPE 消除了 RoPE 的直接外推瓶颈，但不自动保证模型可以 zero-shot 外推到任意长度。** 状态容量、训练长度分布、扫描顺序和数值稳定性仍然会限制实际表现。

### 2.2 成功案例 2：Video Generation 中把视频 token 拉平送入 KDA

直接把视频 token 拉平后送进 KDA 也能训练，[Chimera](https://arxiv.org/abs/2607.28611) 就是这样做的。它使用 temporal-major raster order 把视频序列化，并采用 3:1 的 KDA–MLA 混合骨干，再使用 modality-aware ShortConv 注入局部结构。
...
这里把“sharp 的地方就是 top-K”当作设计直觉是合理的，但不是严格恒等式。sharpness 是整段分布的性质，top-K 是路由器的离散选择结果；block pooling 还可能把块内尖峰抹平。因此最终仍要看路由召回率以及非 top-K 区域究竟有多平坦。
...
![SLA、PISA 与 PWT 逐步结合 Sparse 和 Linear Attention](media/sparse-linear-attention-long-context/03-sla-pisa-pwt.png "图 3｜SLA 用两个分支和 projection 融合；PISA 在同一个 softmax 中组合 Exact Top-K 与一阶 Linear Tail；PWT 再加入二阶修正。来源：OpenAI 内置图像生成能力按 littlewei 的笔记生成，并根据方法公式修正 SLA 分支；版权：littlewei，All Rights Reserved")
...
1. **Sparse Attention 的本质是选择。** 用便宜路由器找到 top-K，在入选位置做精确 attention；主要风险是路由漏选，以及庞大长尾被整体删除。
2. **Linear Attention 的本质是压缩。** 它不显式删除某个历史位置，而是把历史压入固定大小的统计量或 state；主要风险是近似误差、状态冲突和精确 retrieval 能力不足。
3. **Vanilla → Delta → GDN → KDA 改善的是状态管理。** 它们从只累加，发展到会修改、会全局遗忘，再到逐通道遗忘；但这并没有让固定状态等价于保存全部 token。
4. **Sparse 和 Linear 的误差结构互补。** Sparse 擅长精确尖峰却丢掉长尾，Linear 擅长平坦长尾却低估尖峰。
5. **当前更可信的工程答案是 Hybrid。** 层间可以采用 KDA + MLA；层内可以采用 SLA、PISA、PWT 式的 Exact Top-K + Approximate Tail。
6. **系统速度必须拆开看。** 理论复杂度、attention kernel speedup、路由成本和端到端 speedup 是不同指标，不能混在一起比较。
...
1. 本文聚焦 Sparse 与 Linear 两条路线，并不是高效长上下文方法的完整分类。
2. 不同论文的模型规模、序列长度、稀疏率、硬件和质量指标不同，速度数字不能直接横向排名。
3. “sharp 区域通常由 top-K 覆盖”只是直觉，最终取决于路由器的召回率和 block pooling 是否抹掉局部尖峰。
4. Linear/KDA 的固定状态容量仍是根本限制；遗忘机制只能更好地管理容量，不能让容量无限增长。
5. PISA/PWT 即使不需要微调，也仍需要专门 kernel、统计量缓存和数值稳定处理；training-free 不等于 engineering-free。
6. PWT 和 frame-level video Delta update 都还需要更大规模、更独立的实验验证。

## 我的判断

我现在对 long-context attention 的理解，不再是“找到一个便宜算子完全替代 Full Attention”，而是“把有限的精确计算预算放到最值得的位置”。

Sparse Attention 的价值，是把精确 softmax 留给少数关键位置；Linear/KDA 的价值，是用固定成本持续传播全局状态。两者都不是完整 attention 的无损替代，但它们放在一起以后，缺点恰好能够互相补偿。

因此，我最想保留的结论仍然是：

> **尖峰精算，长尾近似。**

在模型层间，KDA 负责 global states 的传播，MLA 负责 fine-grained retrieval；在同一个 attention layer 内，top-K 走 sparse exact softmax，剩余平坦区域走一阶或二阶 Linear/Taylor approximation。

这两个层次的 hybrid 并不冲突。未来完全可能出现这样的组合：模型的大多数层使用 KDA，只在少数 MLA layers 中计算 attention；而这些 MLA layers 本身又使用 PISA/PWT 式的 sparse–linear 分解。

对视频而言，我会把 Chimera 当成“把视频拉平成序列后，KDA 已经是一个很强的可行基线”，但不会把它理解成“视频本来就是一维序列”。帧内排列不变、同时保持时间因果和高并行效率的 video-native update，可能是接下来更值得研究的问题。

## 下次只看这些

1. **两类方法：** Sparse Attention 选择并精算少数位置；Linear Attention 把全部历史压进固定状态。
