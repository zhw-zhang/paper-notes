...
公开网站页眉的“管理”菜单提供三个不依赖本地命令行的入口：

- “新建公开笔记”会在 GitHub 文件编辑器中打开一份已经填好日期、时间、许可字段和七个固定章节的模板。
- 打开任意笔记后，阅读工具栏的“在线编辑”会精确打开对应 Markdown。
- “管理公开笔记”和“打开完整网页编辑器”分别用于浏览文件列表和进入 `github.dev` 工作台。

完成修改后，在 GitHub 中提交到 `main`；Pages 会自动校验并重新发布。网站不保存 GitHub Token，写入权限完全由 GitHub 账号控制。普通访客可以查看或提出 Pull Request，但没有仓库写权限就不能直接修改正式内容。

> [!CAUTION]
> 网页新建入口直接写入公开仓库，因此只适合准备公开的笔记。私人草稿仍应保存在被 Git 忽略的本地 `content/private/`，不要在网页编辑器中创建或提交敏感内容。
...
如果只说“先起草”，技能会把笔记保存在被 Git 忽略的 `content/private/`，保留 `sharing: "private"` 且不会推送。
...
填写书目信息、阅读时间、创建/编辑时间、标签、one-liner、论文许可、笔记许可和分享状态。正文固定保留七个章节：`研究问题`、`核心方法`、`关键发现`、`我的提问`、`局限与疑问`、`我的判断`、`下次只看这些`。
