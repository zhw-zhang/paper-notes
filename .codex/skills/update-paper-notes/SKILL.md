...
10. Add figures only when they materially shorten recall. Verify reuse permission from an authoritative source. Keep every visible caption short: figure number, a brief description, and `来源：论文名或单位名` are enough; do not append license or copyright-status wording. If permission is unclear, link to the original instead of copying the image.
...
12. Never add or hand-edit reading-time metadata. The site build computes `reading_minutes` automatically from the note body. Preview locally when layout or formulas changed. Do not run content validation.
